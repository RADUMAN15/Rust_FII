...
fn rot13() -> Result<(), io::Error> {
    ...
    for c in s.chars() {
        ...
        ch = c as u8;
        if !c.is_ascii(){
            return Err(io::Error::new(io::ErrorKind::InvalidData, "Char encountered is not ASCII"));
        } else {
            ...
            else {  //am orice ascii in loc de litere (nu cast ch la u8 pt a nu pierde biti)
                result.push(c);
            }
        }
    }
